...
execution_updated_at: "2026-09-05T12:55:17Z"
execution_completed_at: "2026-09-05T12:55:17Z"
current_step: null
---
...
| PLAN-00002-STEP-08 | completed | 2026-09-04T20:47:00Z | 2026-09-04T20:51:51Z | Sdist inspection found the leak far worse than reviewed (`.opencode/node_modules/`, `docs/reviews/`, `docs/plans/`, `AGENTS.md`, `.python-version` all shipped); explicit include list fixes it — forbidden-path assertion passes locally; wheel smoke test passes in a clean venv; bare `pytest` ungated, explicit flags enforce 100%; full suite 751 passed with the relocated gate | `[tool.hatch.build.targets.sdist]` explicit include (code, tests, vectors, user docs, lock); new `wheel-test` CI job (public-symbol sweep, `py.typed`, `__version__`, NIST round-trip from the installed wheel); sdist contents assertion in the build job; coverage flags moved from `addopts` to `just coverage` + CI's explicit flags |
| PLAN-00002-STEP-09 | completed | 2026-09-04T20:52:00Z | 2026-09-04T21:00:39Z | All inspection checks pass: no conformance "proven" claims (2 remaining "proof" matches are the disclaimer sentence and "vector-provenance" — correct usage); radix subset in README table + features + AGENTS.md + configuration docs; five security notes + labelled zero key; four badges; all six community files present; built METADATA shows Documentation URL and noreply author/maintainer; full fast suite 749 passed, 100% coverage, pyright strict clean | Docs-only step: README security notes + calibrated language + migration rewrite (L6), SECURITY.md 1.x lifecycle, AGENTS.md stale line + subset note, configuration length properties (L3), CONTRIBUTING/CODE_OF_CONDUCT/issue+PR templates, badges, Documentation URL, backlog updated with version-policy revision (REQ-24) |
| PLAN-00002-STEP-10 | completed | 2026-09-04T21:10:00Z | 2026-09-04T21:26:56Z | `just bench` runs end-to-end, both tables printed (output in Verification results); fresh numbers recorded with divergence noted per the stop condition; full fast suite 749 passed, 100% coverage, pyright strict clean | `benchmarks/timing.py` reproduces the SECURITY.md methodology (25 batches, adaptive inner size, all-zero vs all-max) and the throughput table; README gains the Performance section with today's measured numbers and the quadratic note; SECURITY.md references the harness; `just bench` recipe; directory-structure and developer-guide updated |
| PLAN-00002-STEP-11 | completed | 2026-09-04T23:00:00Z | 2026-09-05T11:14:14Z | Full matrix green on the release path: run 33961464777 (`41dd6cd`) and run 33962272945 (`ff97296`, the tagged release commit) both **success** — 9 quality legs incl. Windows, audit, build + sdist assertion, wheel-test. Three CI defects found and fixed en route (checkout SHA typo, pip-audit/gitleaks step bugs, Windows cp1252 fixture read); two folded items (REQ-13, REQ-21 L1) delivered per user approval | All local gates also green (`just build`, `just secrets`, `pre-commit run --all-files`); justfile `$$` latent defect fixed |
| PLAN-00002-STEP-12 | completed | 2026-09-05T12:16:01Z | 2026-09-05T12:55:17Z | Signed annotated tag `v1.0.0` → `ff97296` (verified locally: `gpg: Good signature from Joseph Lee <joelee@users.noreply.github.com>`); GitHub Release published (title/notes as drafted); Publish run 33965555386: gate invoked at `refs/tags/v1.0.0`, **success** (~10.5 min); PyPI JSON confirms 1.0.0 live — Production/Stable, `Requires-Python: >=3.12`, `cryptography>=50.0.0`, both artifacts, not yanked, README rendering verified | Tag, release, and push executed by the user; Builder performed the post-publish verification and this record. Publish path exercised `pypi-publish@v1.14.2` and the tag-guard step for the first time, both clean |
...
| 2026-09-04T20:51:51Z | PLAN-00002-STEP-08 | Step completed: sdist contents pinned via explicit `[tool.hatch.build.targets.sdist]` include (the pre-change sdist shipped `.opencode/node_modules/`, `docs/reviews/`, `docs/plans/`, `AGENTS.md`, `.python-version` — worse than either review recorded); build job asserts forbidden-path absence; new `wheel-test` job installs the built wheel into a clean venv and exercises the public surface; coverage flags relocated from `addopts` to `just coverage` and CI's explicit flags (bare `pytest` ungated for packagers, floor still enforced on every CI execution); developer guide updated | Verification results table below | Proceed to PLAN-00002-STEP-09 |
| 2026-09-04T21:00:39Z | PLAN-00002-STEP-09 | Step completed: README gains a five-point "Security notes" section after the quick start (determinism, confidentiality-only, wrong-key silence, domain floor, redacted errors) with the zero key replaced by a secret-store call and a tweak-usage comment; trust-section language calibrated ("verified against", "tested exhaustively for these two domains", explicit no-independent-audit disclaimer); radix documented as a deliberate subset of NIST's inclusive range in the comparison table, features list, AGENTS.md, and configuration docs; migration minimum-domain paragraph rewritten (L6); SECURITY.md 1.x lifecycle table; AGENTS.md stale pre-1.0 line fixed; `min_length`/`max_length` documented (L3); CONTRIBUTING.md, CODE_OF_CONDUCT.md, issue/PR templates, badges, Documentation URL, noreply author/maintainer metadata; backlog records the version-policy revision | Verification results table below | Proceed to PLAN-00002-STEP-10 |
| 2026-09-04T21:26:56Z | PLAN-00002-STEP-10 | Step completed: `benchmarks/timing.py` committed (value-dependent table reproducing the SECURITY.md methodology; throughput table); `just bench` recipe added; README Performance section carries today's measured numbers with the quadratic-past-~1,000 note; SECURITY.md timing table references the harness; directory-structure and developer-guide updated | Verification results table below | Proceed to PLAN-00002-STEP-11 |
| 2026-09-04T23:16:16Z | PLAN-00002-STEP-11 | Release preparation implemented (local half): `version = "1.0.0"` + `Development Status :: 5 - Production/Stable` + `uv lock` self-reference refresh; CHANGELOG gains the `[1.0.0]` section (every user-visible change from this plan) plus the Keep a Changelog link reference definitions and the REQ-24 version-policy revision in the header; REQ-13 folded in per user approval — `.pre-commit-config.yaml` (ruff-pre-commit pinned to SHA `321478e` v0.16.6, ruff-format, local gitleaks hook pinned to v8.30.1's SHA `83d9cd6`) with `pre-commit run --all-files` green; REQ-21 L1 folded in — `.codegraph/.gitignore` untracked, `.codegraph/` added to the root `.gitignore`; latent `$$`-escaping defect in the `secrets` justfile recipe fixed (just 1.58.0 does not unescape `$$`; single `$` is correct). The STEP-11 doc task "backlog marks 1.0 shipped" is deferred to STEP-12's post-publish follow-up per that step's own sequencing: a "shipped" claim before the tag and publish would be false | Verification results table below | Push + full-matrix confirmation pending (user action); then STEP-12 |

### Deviations and blockers

| Timestamp (UTC) | Step | Deviation or blocker | Impact | Decision required from |
|---|---|---|---|---|
| 2026-09-04T21:26:56Z | STEP-10 | Plan gap discovered: `REQ-13` (`.pre-commit-config.yaml`) and `REQ-21` L1 (untrack `.codegraph/`) appear in the requirement catalogue and AC-12 but are not assigned to any step's `Requirements:` line or implementation tasks — no step is authorised to implement them | The completion hand-off requires every requirement mapped to evidence; STEP-11/12 cannot truthfully complete without these two small items landing somewhere | User (see checkpoint message: fold into STEP-11 with recorded approval, or return to Delivery Planner) |
| 2026-09-04T23:16:16Z | STEP-11 | RESOLVED: user approved Option 1 — fold REQ-13 and REQ-21 L1 into STEP-11 with this recorded approval. Both implemented in STEP-11 (`.pre-commit-config.yaml` committed, `pre-commit run --all-files` green; `.codegraph/.gitignore` untracked and `.codegraph/` covered by the root `.gitignore`) | Gap closed; no Planner revision required | User (approved 2026-09-04T23:00Z) |
| 2026-09-04T23:58:33Z | STEP-11 | CI failure on the pushed branch investigated (run 33930558164, all 11 jobs failed in ~13s): **Builder's own transcription error** — the checkout SHA pinned in STEP-07 was `11bd5960…` instead of the API-returned `11d5960a…` (v4.2.2's prefix `11bd7190…` conflated with the v4 target during hand-transcription); the accompanying comment "v4.3.0" was also asserted without evidence (the commit is v4.4.0). NOT caused by the user's `c095d88` (adds docs/reviews reports only). Two further latent workflow defects found during the investigation and fixed: `pip-audit -r uv.lock` (uv.lock is TOML — replaced with `uv export --frozen --no-hashes --no-emit-package fpr-ff1 --format requirements-txt`), and the gitleaks `sha256sum -c` filename mismatch (tarball now downloaded under its canonical name). All five pins re-verified against api.github.com, and both audit commands plus the checksum sequence tested locally green | Fixed in the follow-up commit; the user's own stop-condition framing ("wrong SHA → stop and report") is why this is being reported explicitly rather than silently patched | User informed; Builder fixed |
| 2026-09-05T00:20:00Z | STEP-11 | Second CI run investigated (33931985927): the SHA fix worked (checkout resolves; Dependency audit PASSED including both new audit steps; Secret scan PASSED), but all 9 matrix legs failed at **Format check**. Root cause this time IS the user's `c095d88`: the committed review reports are scanned by ruff 0.16's Markdown code-block formatting, and review 00003's quoted code sketches (the `__version__` and pickling examples) are not ruff-formatted. Resolution chosen: `extend-exclude = ["docs/reviews"]` in pyproject — review reports are verbatim records of external documents and are preserved as authored (same principle as the NIST fixtures), rather than reformatting quoted evidence; local gates re-verified green with the exclusion | Verbatim evidence preserved; formatter still owns all project-authored files; alternative (reformatting the reviews) rejected as evidence tampering | User informed |
| 2026-09-05T10:08:45Z | STEP-11 | Third CI run investigated (33933879294 — note: this run is Dependabot's `uv` PR against the squashed main, i.e. post-merge of PR #1 as `28ac9d4`): all 6 Linux/macOS legs fully green including the complete oracle suite; all 3 **Windows legs fail at "Test with 100% coverage gate"**. Root cause: `tests/test_frozen_kat.py` read `oracle_kat_frozen.json` (written with `ensure_ascii=False`, containing non-BMP radix-256/65535 alphabet characters) via a bare `open()` — on Windows Python ≤3.14 the default text codec is the locale (cp1252), which raises `UnicodeDecodeError`; Linux/macOS default to UTF-8, hence the OS split. Reproduced locally by forcing a cp1252 decode (fails at byte 0x90). The NIST fixtures are pure ASCII, which is why the v0.1.1 Windows matrix was green and this surfaced only with the new KAT file. Fix: explicit `encoding="utf-8"` on all five fixture I/O sites (test_frozen_kat, conftest, test_nist_vectors, test_intermediates, and the generator's `write_text` — which would have corrupted the file if run on a Windows machine) | Windows-only defect closed before tagging; local gates re-verified green (749 passed, 100% coverage, pyright/ruff clean) | User informed; fix committed to main |
| 2026-09-05T12:55:17Z | PLAN-00002-STEP-12 | Release executed and verified: user created the signed annotated `v1.0.0` tag on `ff97296` (GPG key `9C23C2BA…`, uid `joelee@users.noreply.github.com`, registered with GitHub), pushed it, and published the GitHub Release; the release event triggered the Publish workflow (run 33965555386) which gated at the tag, downloaded the gated artifact, verified tag==version, twine-checked, and published via Trusted Publishing; Builder verified PyPI state post-publish | Verification results table below | Plan complete — closure records committed |
...
| 2026-09-04T20:51:51Z | PLAN-00002-STEP-08 | `tar -tzf` inspection of the pre-change sdist | Shipped `.opencode/node_modules/` (thousands of files), `docs/reviews/`, `docs/plans/`, `AGENTS.md`, `.python-version` | Leak confirmed — worse than review 00003 L2 / 00004 LOW-02 recorded |
| 2026-09-04T20:51:51Z | PLAN-00002-STEP-08 | `tar -tzf` inspection of the post-change sdist + forbidden-path assertion | 41 files, all intended; `AGENTS.md`, `docs/plans/`, `docs/reviews/`, `.opencode/`, `.python-version`, `.codegraph` all absent | Sdist contents pinned and verified |
| 2026-09-04T20:51:51Z | PLAN-00002-STEP-08 | Wheel smoke test in a clean venv (`uv pip install dist/*.whl`, then public-symbol sweep, `py.typed`, `__version__`, NIST round-trip) | Passed | The CI `wheel-test` job's script verified locally |
| 2026-09-04T20:51:51Z | PLAN-00002-STEP-08 | Bare `uv run pytest -m 'not slow' -q` (no coverage flags) | 749 passed, no coverage gate | Packager path ungated as intended |
| 2026-09-04T20:51:51Z | PLAN-00002-STEP-08 | `uv run pytest -q --cov=fpr_ff1 --cov-report=term-missing --cov-fail-under=100` (full suite incl. slow) | 751 passed in 214.33s; 100% line and branch coverage | Floor enforced via relocated flags |
| 2026-09-04T20:51:51Z | PLAN-00002-STEP-08 | `uv run pyright` (strict) + `uv run ruff format --check .` + `uv run ruff check .` + ci.yml YAML parse | 0 errors; 33 files formatted; all checks passed; valid YAML | Clean |
| 2026-09-04T21:00:39Z | PLAN-00002-STEP-09 | README/AGENTS/SECURITY/configuration inspection script (REQ-15/16/17/18/19/21 assertions) | All pass; 2 remaining "proof" matches are the disclaimer sentence and "vector-provenance" — calibrated usage, not conformance claims | Every requirement item verifiably in place |
| 2026-09-04T21:00:39Z | PLAN-00002-STEP-09 | `uv run pytest -m 'not slow' -q --cov=... --cov-fail-under=100` + pyright + ruff | 749 passed; 100% coverage; 0 errors; 38 files formatted; all checks passed | No regression |
| 2026-09-04T21:00:39Z | PLAN-00002-STEP-09 | `uv build` + `uvx twine check` + METADATA inspection | Both artifacts PASSED; Documentation URL, noreply author/maintainer present | Packaging metadata confirmed |
| 2026-09-04T21:26:56Z | PLAN-00002-STEP-10 | `uv run python benchmarks/timing.py` (via `just bench` methodology) | Value-dependent deltas: (10,10) +2.3%, (10,60) +0.5%, (10,200) +6.1%, (256,32) −1.8%, (65535,12) +1.7% — all single-digit, one negative, consistent with SECURITY.md's "noise floor" characterisation. Throughput: 30.9 µs/op (6 numerals), 1.6 µs construction, 1.2/1.6/7.0/26.8 µs per numeral at n=100/1,000/5,000/20,000 | Divergence from review 00003 M10's numbers noted: this machine is uniformly faster; construction dropped 8.8 → 1.6 µs largely because STEP-04 removed the ECB-encryptor construction from `__init__`; the quadratic climb past ~1,000 reproduces exactly. Fresh numbers used in the README table; stale claims not copied |
| 2026-09-04T21:26:56Z | PLAN-00002-STEP-10 | `uv run pytest -m 'not slow' -q --cov=... --cov-fail-under=100` + pyright + ruff | 749 passed; 100% coverage; 0 errors; 39 files formatted; all checks passed | Harness not collected by pytest; no regression |
| 2026-09-04T23:16:16Z | PLAN-00002-STEP-11 | `uv run pre-commit run --all-files` | ruff (legacy alias) Passed; ruff format Passed; gitleaks Passed | REQ-13 acceptance evidence. Note: the upstream `gitleaks-system` hook id mangles flag passing under this pre-commit version; a `repo: local` hook with the identical entry works and is what shipped. Hook rev SHAs resolved via api.github.com (ruff-pre-commit v0.16.6 → `321478e58f4938179c6b86e4ddfa923d1547a49b`; gitleaks v8.30.1 → `83d9cd684c87d95d656c1458ef04895a7f1cbd8e`), accessed 2026-09-04 |
| 2026-09-04T23:16:16Z | PLAN-00002-STEP-11 | `git ls-files .codegraph` | Empty (was `.codegraph/.gitignore`) | REQ-21 L1 acceptance evidence; worktree stays clean via root `.gitignore` |
| 2026-09-04T23:16:16Z | PLAN-00002-STEP-11 | `just build` (quality gate + uv build) | 751 passed in 213.89s with 100% line-and-branch coverage; format/lint/pyright clean; `fpr_ff1-1.0.0.tar.gz` + wheel built | Full local gate green on the release state |
| 2026-09-04T23:16:16Z | PLAN-00002-STEP-11 | `uvx twine check dist/*` + wheel METADATA inspection | Both artifacts PASSED; `Version: 1.0.0`, `Classifier: Development Status :: 5 - Production/Stable`, `Requires-Python: >=3.12`, `Requires-Dist: cryptography>=50.0.0` | Release metadata confirmed |
| 2026-09-04T23:16:16Z | PLAN-00002-STEP-11 | `just secrets` | Scanned ~98 MB in 1.66s, no leaks found, exit 0 (after fixing the `$$` escaping defect) | Clean |
| 2026-09-04T23:58:33Z | PLAN-00002-STEP-11 | CI run 33930558164 investigated: error `Unable to resolve action actions/checkout@11bd5960…` on all 11 jobs | Root cause: transcription typo in STEP-07's pin (correct SHA `11d5960a326750d5838078e36cf38b85af677262` = checkout v4.4.0, confirmed via api.github.com matching-refs); other four pins verified character-exact against the API | Fixed: checkout SHA + version comment corrected in ci.yml (6×) and publish.yml (1×) |
| 2026-09-04T23:58:33Z | PLAN-00002-STEP-11 | `uv export --frozen --no-hashes --no-emit-package fpr-ff1 --format requirements-txt` + `uvx pip-audit -r …` (tested locally) | "No known vulnerabilities found", exit 0 | Replaces the broken `pip-audit -r uv.lock` (uv.lock is TOML); fpr-ff1 omitted from the audit (unpublished local project, nothing to audit) |
| 2026-09-04T23:58:33Z | PLAN-00002-STEP-11 | Minimum-set audit tested locally: venv with `cryptography==50.0.0` + pip-audit, run via the venv's own `bin/pip-audit` | "No known vulnerabilities found", exit 0 | Replaces the invalid `pip-audit --python` invocation (flag does not exist); auditing exactly the declared runtime floor |
| 2026-09-04T23:58:33Z | PLAN-00002-STEP-11 | Gitleaks install sequence tested locally (download canonical-named tarball, `sha256sum -c`, extract) | `gitleaks_8.30.1_linux_x64.tar.gz: OK` | Fixes the `sha256sum -c` filename mismatch; exec-format error on arm64 macOS is expected — CI runs ubuntu-x64 |
| 2026-09-04T23:58:33Z | PLAN-00002-STEP-11 | Both workflow files re-validated (YAML parse) + full pin inventory re-checked | Valid; 5 distinct SHAs, all matching api.github.com responses | Ready for re-push |
| 2026-09-05T00:20:00Z | PLAN-00002-STEP-11 | CI run 33931985927 jobs API: per-step conclusions | Dependency audit: all steps success (23s); Secret scan: success (4s); all 9 matrix legs: failure at step 6 "Format check" (~1s), subsequent steps skipped | Pin fix confirmed working; failure isolated to ruff Markdown formatting |
| 2026-09-05T00:20:00Z | PLAN-00002-STEP-11 | `uv run ruff format --check .` locally at HEAD | 1 file would be reformatted: `docs/reviews/00003-…md` (quoted code sketches) | Diagnosis confirmed |
| 2026-09-05T00:20:00Z | PLAN-00002-STEP-11 | After `extend-exclude = ["docs/reviews"]`: format check, lint, pyright strict, fast suite with coverage gate | 39 files formatted; all checks passed; 0 errors; 749 passed, 100% coverage | Fix verified locally |
| 2026-09-05T10:08:45Z | PLAN-00002-STEP-11 | CI run 33933879294 jobs API: per-step conclusions | Secret scan + Dependency audit: success; 6 Linux/macOS quality legs: success (full suite incl. oracle); 3 Windows quality legs: failure at "Test with 100% coverage gate"; build/wheel-test skipped | OS-isolated test failure identified |
| 2026-09-05T10:08:45Z | PLAN-00002-STEP-11 | Local reproduction: `open('tests/vectors/oracle_kat_frozen.json', encoding='cp1252').read()` | `UnicodeDecodeError: 'charmap' codec can't decode byte 0x90` — deterministic on the Windows locale codec; file confirmed to contain non-ASCII/non-BMP bytes; NIST fixtures confirmed pure ASCII | Root cause proven, not inferred |
| 2026-09-05T10:08:45Z | PLAN-00002-STEP-11 | After `encoding="utf-8"` on all fixture I/O: targeted vector suites, full fast suite with coverage gate, pyright, ruff | 40 vector tests passed; 749 passed; 100% coverage; 0 errors; 39 files formatted | Fix verified locally |
| 2026-09-05T12:55:17Z | PLAN-00002-STEP-11 | CI matrix confirmation (REQ-22): runs 33961464777 (`41dd6cd`) and 33962272945 (`ff97296`) | Both **success** — full 9-leg matrix incl. Windows, dependency audit, secret scan, build + sdist assertion, wheel-test | Release-content and tagged-commit matrices green |
| 2026-09-05T12:55:17Z | PLAN-00002-STEP-12 | `git tag -v v1.0.0` (local) | Good signature from `Joseph Lee <joelee@users.noreply.github.com>` [ultimate], EDDSA key `9C23C2BA97E234C0CEBEF95A7D1CDEA67B7F5BD8`; annotated tag object → commit `ff972965d2df25d3ed02e13ebd0949d5c529cf1d` | Signed annotated tag (REQ-23) |
| 2026-09-05T12:55:17Z | PLAN-00002-STEP-12 | Publish run 33965555386 (event: release, head `v1.0.0`) | **Success**; reusable gate invoked at `refs/tags/v1.0.0`; ~10.5 min gate-to-publish; first exercise of `pypi-publish@v1.14.2` and the tag-guard step, both clean | The published artifacts are the gated, twine-checked artifacts |
| 2026-09-05T12:55:17Z | PLAN-00002-STEP-12 | PyPI JSON API (`pypi.org/pypi/fpr-ff1/json`) | `"version": "1.0.0"`; classifiers include `Development Status :: 5 - Production/Stable`; `requires_python: ">=3.12"`; `requires_dist: cryptography>=50.0.0`; wheel (20,292 B) + sdist (849,626 B) present, `yanked: false`; README description renders badges, security notes, radix-subset table, performance section, calibrated language; author/maintainer noreply; Documentation URL present | All AC-14 metadata checks pass |
| 2026-09-05T12:55:17Z | PLAN-00002-STEP-12 | Sdist integrity cross-check (size anomaly investigation) | Local rebuild from `ff97296` produces a byte-identical 849,626-byte sdist, 39 files, zero forbidden paths — the size is legitimate content (per-round intermediates JSON, KAT vectors, uv.lock), and matches the CI sdist-assertion job's pass | No leak; published artifact equals the pinned include list |
| 2026-09-05T12:55:17Z | PLAN-00002-STEP-12 | Attestations | Publish via `pypi-publish@v1.14.2` with default `attestations: true` under OIDC; a failed attestation upload fails the job, and the run succeeded (same path as the attested 0.1.x releases); wheel carries the `core-metadata` attestation digest in the JSON API. Visible on the PyPI project page's provenance dialog | Attestation generation confirmed as part of the successful publish |

### Completion summary

- **Implementation status:** `completed`
- **Completed requirements:** All 24 (`REQ-01` through `REQ-24`, including the user-approved folded items REQ-13 and REQ-21 L1, and REQ-24's changelog text)
- **Incomplete requirements:** None
- **Outstanding blockers:** None
- **Review request:** Ready — all 14 acceptance criteria evidenced (AC-01–AC-12 via the committed test suite and verification table; AC-13 via the benchmark harness and README performance section; AC-14 via the signed tag, green publish run, and PyPI verification above)
