...
execution_updated_at: "2026-09-07T21:44:54Z"
execution_completed_at: "2026-09-07T21:44:54Z"
current_step: null
---
...
| PLAN-00003-STEP-14 | completed | 2026-09-07T15:19:53Z | 2026-09-07T16:17:57Z | Staged checkpoint reviewed and approved by user; digest `60e00767...cbe80` verified at continuation | Platform wheel + sdist fallback proven locally; CI wired; REQ-19 proven end-to-end |
| PLAN-00003-STEP-15 | completed | 2026-09-07T16:17:57Z | 2026-09-07T21:44:54Z | Staged checkpoint reviewed and approved by user; digest `e2e36931...e758d` verified at continuation | 2.0.0rc1 docs + release prep; all three artifacts build; plan closed |
...
| 2026-09-07T16:06:22Z | PLAN-00003-STEP-14 | STEP-14 implemented and verified (locally verifiable scope). **External pins fetched and recorded** (work-log Sources): dtolnay/rust-toolchain @ `d1031067263f94b142dd6c0ce24c5eb9d02d52a0` (GPG-verified master head, 2026-09-03); cargo-audit 0.22.2; maturin 1.15.0. **Architecture (bounded design correction recorded):** the extension moved INSIDE the package as `fpr_ff1._rs` (standard maturin mixed layout) — maturin's `python-source` semantics rejected the top-level `_fpr_ff1_rs` placement, and the in-package name is better citizenship anyway (no top-level namespace on PyPI; the pure package still never imports it). The Rust `#[pymodule]` renamed `_fpr_ff1_rs` → `_rs` (PyInit symbol follows); `_RUST_MODULE = "fpr_ff1._rs"` in `_ff1.py`; conftest/test/timing imports migrated; the dev pseudo-dist `rust/fpr-ff1-rust/pyproject.toml` DELETED — `maturin build -m rust/fpr-ff1-rust/Cargo.toml` now resolves the ROOT pyproject (name fpr-ff1, single version source) and `[tool.maturin]` (module-name fpr_ff1._rs, python-source src, pyo3/abi3-py312) drives the publishable wheel. **Dev loop rewritten:** `just backend-dev` = `PYO3_PYTHON=<abs>/.venv/bin/python cargo build --release` + copy the cdylib to `src/fpr_ff1/_rs.so` (gitignored) — no pip involvement, so uv sync can never strip the extension (the old `maturin develop` would now have clobbered the editable install). Two real bugs found and fixed during the step: (1) plain `cargo build` linked against a Python 3.14 interpreter on this machine → `undefined symbol: PyIter_NextItem` at import; fixed by the absolute `PYO3_PYTHON` (relative paths break — cargo build scripts run with the crate dir as cwd); (2) my first wheel-smoke used a mis-remembered NIST ciphertext — caught immediately because both backends agreed with each other but not my constant; re-driven from the authoritative vector file (all 9 vectors, both backends). **Local verification:** platform wheel builds as `fpr_ff1-1.1.0-cp312-abi3-manylinux_2_34_x86_64.whl` (full package + `fpr_ff1/_rs.abi3.so`, maturin auto-manylinux tag, SBOM included); scratch-venv install reproduces all 9 NIST vectors on BOTH backends; sdist (hatchling, `rust/` included for wheel builders, forbidden-pattern scan clean) installs pure-Python with NO Rust invocation and `backend="rust"` raises the documented `BackendError` — REQ-19 proven end-to-end. **CI:** ci.yml gains `wheel-build` (5-target D5 matrix: linux x86_64/aarch64, macOS arm64/x86_64-cross, Windows x64; SHA-pinned rust-toolchain; `uvx maturin@1.15.0`; twine check; wheels artifact), `wheel-test-platform` (linux wheel into clean venv, both backends vs NIST), `sdist-test` (REQ-19 contract in CI), and a cargo-audit step (0.22.2, --locked, over rust/Cargo.lock) in the audit job; publish.yml downloads the gated wheels artifact and publishes it under the publish-the-tested-artifact rule. `rust-toolchain.toml` (stable, minimal) pins the toolchain. Both workflows YAML-validated. **Gates:** full dual-backend suite 1,401 passed, 100% line/branch coverage; ruff/pyright clean; cargo test 8 green. **Honest limitation:** CI-execution evidence (the plan's "CI green end-to-end on a branch") requires a push — Builder never pushes; all locally-verifiable evidence is green and the CI changes are configuration-reviewed. The user's push of this branch (or the STEP-15 release flow) produces the matrix evidence. Checkpoint staged for review: implementation paths `.github/workflows/ci.yml`, `.github/workflows/publish.yml`, `.gitignore`, `benchmarks/timing.py`, `justfile`, `pyproject.toml`, `rust-toolchain.toml`, `rust/fpr-ff1-rust/pyproject.toml` (deleted), `rust/fpr-ff1-rust/src/lib.rs`, `src/fpr_ff1/_ff1.py`, `tests/conftest.py`, `tests/test_backend_dispatch.py`, `tests/test_rust_aes_validation.py`; proposed commit subject `build: complete PLAN-00003-STEP-14 - Packaging: wheels, CI, sdist fallback`. Staged-diff SHA-256 for the implementation paths (`.github/workflows/ci.yml`, `.github/workflows/publish.yml`, `.gitignore`, `benchmarks/timing.py`, `justfile`, `pyproject.toml`, `rust-toolchain.toml`, `rust/fpr-ff1-rust/pyproject.toml` (deleted), `rust/fpr-ff1-rust/src/lib.rs`, `src/fpr_ff1/_ff1.py`, `tests/conftest.py`, `tests/test_backend_dispatch.py`, `tests/test_rust_aes_validation.py`): `60e00767992f3d99d7126ea905b222ad41b1605c9fb4beb3ea8e808ecd2cbe80` | Verification results below | Awaiting user review; on continuation, commit STEP-14 and begin STEP-15 |
| 2026-09-07T16:17:57Z | PLAN-00003-STEP-14 | User approved the staged checkpoint ("Step 14 approved. Update plan to release as v2.0.0-rc1. Proceed to the next step and get it ready for me to publish and release."); staged-checkpoint gate re-verified (branch `plan/v2.0.0`, HEAD `a8f620b`, staged set, digest match `60e00767...`, no unstaged/untracked changes); commit authorised. **User decision recorded:** the release version is `v2.0.0-rc1` (a release candidate, not the final `v2.0.0`) — a material change to the plan's REQ-23/D6 (which specified `v2.0.0`), recorded here as the user's explicit instruction; STEP-15 will prepare the rc1 release | This entry | Commit STEP-14, then begin STEP-15 (2.0 documentation and release v2.0.0-rc1) |
| 2026-09-07T21:37:16Z | PLAN-00003-STEP-15 | STEP-15 implemented and verified (documentation + release preparation; the tag/push/publish actions remain the user's). **Version:** `pyproject.toml` → `2.0.0rc1` (PEP 440 canonical — a hyphen would parse as a local version and sort as a final release; the publish guard therefore expects the tag `v2.0.0rc1`), `uv.lock` re-recorded (single-line version diff, `uv lock --check` clean). **Documentation set updated in the same change (docs/AGENTS.md):** README — Features (compiled backend bullet), Roadmap (2.0 shipped as rc1, additive framing), a new **Backends** section with the measured table (6.8×/3.3×/0.21×) and the explicit short-vs-long guidance, API signature + `backend` row, `BackendError` in the exceptions table; SECURITY.md — supported-versions table (2.0.0rc1 ✅) and a constant-time-posture paragraph stating the compiled backend does NOT change the timing posture (no overclaim); architecture.md — two-backend mermaid, `fpr_ff1._rs` module, validation-in-Python boundary, stateless-extension thread-safety; configuration.md — `backend` parameter + dual-backend thread-safety; directory-structure.md — `rust/` workspace, `rust-toolchain.toml`, the new test modules and the AES KAT vector file; developer-guide.md — Rust toolchain requirement, `just rust-test`/`just backend-dev`, the compiled-backend section, the extended CI description; CHANGELOG.md — full 2.0.0rc1 entry (Added/Changed/Unchanged); backlog.md — 2.0 item moved to Completed. **Bug fixed during the step (recorded):** the STEP-14 CI smoke scripts had used the no-tweak vector's ciphertext with the tweaked vector's key/tweak — corrected to `6124200773` in both `wheel-test-platform` and `sdist-test` (would have failed CI). **Gates:** full dual-backend suite 1,401 passed, 100% line/branch coverage; ruff/pyright clean; `uv build` + maturin produce all three artifacts as `fpr_ff1-2.0.0rc1` (sdist, py3-none-any wheel, cp312-abi3-manylinux_2_34_x86_64 wheel). **Release hand-off (user actions, Builder never performs them):** merge to main, push, wait for the full matrix (now including wheel-build/wheel-test-platform/sdist-test/cargo-audit) to go green, then tag `v2.0.0rc1` and create the GitHub release — `publish.yml` re-runs the gate and publishes the gated sdist + pure wheel + platform wheels with attestations. Checkpoint staged for review: implementation paths `README.md`, `SECURITY.md`, `CHANGELOG.md`, `docs/architecture.md`, `docs/configuration.md`, `docs/developer-guide.md`, `docs/directory-structure.md`, `docs/backlog.md`, `pyproject.toml`, `uv.lock`, `.github/workflows/ci.yml`; proposed commit subject `build: complete PLAN-00003-STEP-15 - 2.0 documentation and release v2.0.0-rc1`. Staged-diff SHA-256 for the implementation paths (`.github/workflows/ci.yml`, `CHANGELOG.md`, `README.md`, `SECURITY.md`, `docs/architecture.md`, `docs/backlog.md`, `docs/configuration.md`, `docs/developer-guide.md`, `docs/directory-structure.md`, `pyproject.toml`, `uv.lock`): `e2e36931ab9f4897ac6714d6a6f288c66ba2c254da3300e5da0da7cac34e758d` | Verification results below | Awaiting user review; on continuation, commit STEP-15 — this closes the plan (Builder execution complete; Review/Validate still required) |
| 2026-09-07T21:44:54Z | PLAN-00003-STEP-15 | User approved the staged checkpoint ("checkpoint approve"); staged-checkpoint gate re-verified (branch `plan/v2.0.0`, HEAD `403ce91`, staged set, digest match `e2e36931...`, no unstaged/untracked changes); commit authorised. **Final step committed — Builder execution complete.** All 15 steps done; `implementation_status` → completed, `execution_completed_at` set, `current_step` → null. The release actions (merge, push, tag `v2.0.0rc1`, GitHub release) remain the user's; Review/Validate is still required before the plan is considered closed | This entry | Commit STEP-15; hand off for Review/Validate |
| 2026-09-07T22:05:00Z | PLAN-00003-STEP-15 | **Post-completion CI fix (user-reported, PR #5).** The `audit` job's "Install Rust toolchain" step failed with `'toolchain' is a required input` (exit 1). Root cause: `dtolnay/rust-toolchain` declares `toolchain` as a `required: true` input and does NOT read `rust-toolchain.toml` (that file is consumed by rustup/cargo themselves, not the action) — confirmed by fetching the pinned action's `action.yml` at `d1031067…`. Both invocations (`audit` and `wheel-build`) omitted the input; local verification never caught it because `cargo test`/`maturin build` run rustup directly, which auto-detects the file. This is the CI-execution gap flagged in the STEP-14 work log (Builder never pushes, so the action path was never exercised). Fix: added `toolchain: stable` to both invocations, matching the `rust-toolchain.toml` channel pin. This is a defect repair to a Builder-owned file, not a scope change; the plan remains `implementation_status: completed` | This entry | Commit the CI fix; re-run CI on PR #5 |
| 2026-09-07T22:49:09Z | PLAN-00003-STEP-15 | **Post-completion CI fix (user-reported, PR #5).** The `quality` job's coverage gate failed at 97.95% (6 missed stmts, 3 partial branches: `_ff1.py` 160-162, 334, 473, 493). Root cause: those lines are the rust dispatch plumbing (`_rust_ff1`, the `backend == "rust"` branches in encrypt/decrypt, and `_load_rust_backend()` in `__setstate__`), which only execute when the compiled extension is importable. CI's `quality` job never builds the extension, so `BACKENDS = ["python"]` only, the rust tests skip (4 skipped), and the plumbing is never exercised — locally it was covered because `_rs.so` was built. The production code is correct; the gap is test coverage of the plumbing. Fix: added a fake-extension test double in `tests/test_backend_dispatch.py` (`_install_fake_rust_backend` via `types.ModuleType` + `sys.modules`, the inverse of the existing missing-extension simulation) plus two tests — `test_rust_dispatch_plumbing_without_real_extension` (asserts `_rust_ff1` routes encrypt/decrypt to the module's functions with the exact args) and `test_rust_unpickle_revalidates_extension` (asserts `__setstate__` re-imports the backend). No production code changed; no `no cover` markers; no coverage weakening. Verified both conditions: extension absent → 854 passed, 100% line/branch (327 stmts, 112 branches, 0 missed); extension present → 1,403 passed, 100%; ruff/pyright clean. This is a defect repair to a Builder-owned test file, not a scope change; the plan remains `implementation_status: completed` | This entry | Commit the coverage fix; re-run CI on PR #5 |
| 2026-09-07T23:21:39Z | PLAN-00003-STEP-15 | **Post-completion CI fix (user-reported, PR #5).** The `wheel-test-platform` job's install step failed with `error: The wheel filename "*linux*x86_64*.whl" is invalid: Must have a version` (exit 2). Root cause: the glob was single-quoted (`'dist-wheels/*linux*x86_64*.whl'`), so bash never expanded it and `uv pip install` received the literal string, which it rejects as an invalid wheel filename. Every other glob in the workflow (lines 119, 128, 192, 219, 314) is unquoted and works; this one was the sole quoted glob. Fix: removed the quotes so bash expands the glob to the single manylinux wheel (`fpr_ff1-2.0.0rc1-cp312-abi3-manylinux_2_34_x86_64.whl`). Verified locally: the unquoted glob installs the wheel into a clean venv and the exact workflow smoke test (both backends reproduce NIST sample 2 `6124200773`) passes. This is a defect repair to a Builder-owned workflow file, not a scope change; the plan remains `implementation_status: completed` | This entry | Commit the glob fix; re-run CI on PR #5 |
| 2026-09-08T08:00:13Z | PLAN-00003-STEP-15 | **Post-completion CI fix (user-reported, PR #5).** The `wheel-test-platform` job STILL failed with the same `invalid: Must have a version` error after the glob-unquoting fix. Deeper root cause (the unquoting was necessary but insufficient): all five `wheel-build` matrix legs upload to the **same artifact name `wheels`**, and `wheel-test-platform` downloaded it with `merge-multiple: false` (the default), so it received a **single arbitrary leg** — the Windows wheel (`fpr_ff1-2.0.0rc1-cp312-abi3-win_amd64.whl`, artifact ID 10035543537, 187,915 bytes), the last leg to finish. The downloaded directory contained only the win_amd64 wheel, which does not match `*linux*x86_64*.whl`, so bash passed the literal glob through to `uv` (an unmatched glob is passed literally), which rejected it. Evidence: the five wheel-build jobs each uploaded a distinct artifact ID (linux x86_64 = 10035531011, linux aarch64 = 10035531790, macos x86_64 = 10035538736, macos arm64 = 10035536588, win_amd64 = 10035543537), and the download log shows `merge-multiple: false` + `Total of 1 artifact(s) downloaded` with the win_amd64 ID. Fix: added `merge-multiple: true` to the `wheels` download in BOTH `ci.yml` (wheel-test-platform) and `publish.yml` (the release download had the same latent bug — it would have published only one platform wheel instead of the full decided set). This is a defect repair to Builder-owned workflow files, not a scope change; the plan remains `implementation_status: completed` | This entry | Commit the merge-multiple fix; re-run CI on PR #5 |
| 2026-09-08T08:43:55Z | PLAN-00003-STEP-15 | **Post-completion CI fix (user-reported, PR #5).** The `wheel-test-platform` job STILL failed with the same error after the `merge-multiple` fix. **True root cause (the two prior fixes treated symptoms):** the pinned `download-artifact` action's `dist/index.js` shows that when `name` is specified, `isSingleArtifactDownload = !!inputs.name` is true and the action ALWAYS downloads exactly one artifact via `getArtifact(name)` — which returns `artifacts[0]`, or the newest (`sort((a,b)=>b.id-a.id)[0]`) when several share the name. `merge-multiple` is only consulted in the `pattern`/`artifact-ids`/no-name branches, so it was silently ignored (the log confirmed `merge-multiple: true` yet still "Downloading single artifact" + "Total of 1 artifact(s) downloaded"). The underlying defect: all five `wheel-build` matrix legs upload to the SAME artifact name `wheels`, and GitHub treats same-named artifacts as versions of one artifact, so `name: wheels` can only ever return the newest leg (the Windows wheel). Fix: (1) `wheel-build` now uploads under a unique per-leg name `wheels-${{ matrix.target }}`; (2) `wheel-test-platform` downloads `name: wheels-x86_64-unknown-linux-gnu` (a name download returns exactly one artifact, so the `*linux*x86_64*.whl` glob matches); (3) `publish.yml` downloads `pattern: wheels-*` with `merge-multiple: true` (the pattern branch DOES honour merge-multiple, collecting all five wheels for the release). Verified: YAML valid for both workflows; the action source at the pinned SHA `3e5f45b2…` was read to confirm the name-vs-pattern behaviour. This is a defect repair to Builder-owned workflow files, not a scope change; the plan remains `implementation_status: completed` | This entry | Commit the unique-artifact-name fix; re-run CI on PR #5 |
...
| 2026-09-07T16:06:22Z | PLAN-00003-STEP-14 | `VIRTUAL_ENV=.venv uvx maturin@1.15.0 build -m rust/fpr-ff1-rust/Cargo.toml --release --out dist` | `fpr_ff1-1.1.0-cp312-abi3-manylinux_2_34_x86_64.whl` built: full package + `fpr_ff1/_rs.abi3.so`, cp312-abi3 tag, maturin auto-manylinux audit, SBOM | The publishable platform wheel, locally proven |
| 2026-09-07T16:06:22Z | PLAN-00003-STEP-14 | Scratch-venv install of the platform wheel | All 9 NIST vectors reproduce on BOTH backends; decrypt round-trips | Wheel is independently installable and correct |
| 2026-09-07T16:06:22Z | PLAN-00003-STEP-14 | `uv build` + sdist contents scan + scratch-venv sdist install | Sdist: 48 files, `rust/` included, forbidden patterns absent; installs pure-Python (no Rust invoked); NIST vector passes; `backend="rust"` raises the documented `BackendError` | **REQ-19 proven end-to-end** |
| 2026-09-07T16:06:22Z | PLAN-00003-STEP-14 | YAML validation of both workflows | ci.yml jobs: quality, audit, build, wheel-build, wheel-test, wheel-test-platform, sdist-test, secrets; publish.yml: quality, publish | Configuration reviewed; execution evidence awaits the user's push |
| 2026-09-07T16:06:22Z | PLAN-00003-STEP-14 | `uv run pytest --cov=fpr_ff1 --cov-report=term-missing --cov-fail-under=100` / ruff / pyright / `cargo test` | 1,401 passed; 100% line/branch coverage; clean; 0 errors; 8 passed | Full local gates after the fpr_ff1._rs migration |
| 2026-09-07T21:37:16Z | PLAN-00003-STEP-15 | `uv lock --check` / `uv run ruff format --check .` / `ruff check .` / `uv run pyright` | Lock consistent; clean; 0 errors, 0 warnings | After the version bump and doc edits |
| 2026-09-07T21:37:16Z | PLAN-00003-STEP-15 | `uv run pytest --cov=fpr_ff1 --cov-report=term-missing --cov-fail-under=100` | 1,401 passed; 100% line/branch coverage (327 stmts, 112 branches, 0 missed) | Full dual-backend gate incl. slow sweeps (257.0 s) |
| 2026-09-07T21:37:16Z | PLAN-00003-STEP-15 | `uv build` + `uvx maturin@1.15.0 build` | All three artifacts built as `fpr_ff1-2.0.0rc1`: sdist, py3-none-any wheel, cp312-abi3-manylinux_2_34_x86_64 wheel | Release-ready artifact naming verified |
| 2026-09-07T22:05:00Z | PLAN-00003-STEP-15 | `uv run python -c "import yaml; yaml.safe_load(open('.github/workflows/ci.yml'))"` | YAML valid | Post-completion CI fix: `toolchain: stable` added to both `dtolnay/rust-toolchain` invocations |
| 2026-09-07T22:49:09Z | PLAN-00003-STEP-15 | `uv run pytest --cov=fpr_ff1 --cov-report=term-missing --cov-fail-under=100` (extension ABSENT, CI condition) | 854 passed, 4 skipped; 100% line/branch (327 stmts, 112 branches, 0 missed) | Coverage floor restored without the built extension |
| 2026-09-07T22:49:09Z | PLAN-00003-STEP-15 | `uv run pytest --cov=fpr_ff1 --cov-report=term-missing --cov-fail-under=100` (extension PRESENT) | 1,403 passed; 100% line/branch (327 stmts, 112 branches, 0 missed) | Full dual-backend gate still green |
| 2026-09-07T22:49:09Z | PLAN-00003-STEP-15 | `uv run ruff format --check .` / `ruff check .` / `uv run pyright` | Clean; 0 errors, 0 warnings | After the fake-extension test addition |
| 2026-09-07T23:21:39Z | PLAN-00003-STEP-15 | `uv venv wheel-check-rs` + `uv pip install --python wheel-check-rs dist/*linux*x86_64*.whl` + the workflow smoke script | Wheel installs into a clean venv; both backends reproduce NIST sample 2 `6124200773`; decrypt round-trips | Unquoted-glob fix verified end-to-end |
| 2026-09-08T08:00:13Z | PLAN-00003-STEP-15 | `uv run python -c "import yaml; yaml.safe_load(...)"` over ci.yml + publish.yml | YAML valid | merge-multiple fix applied to both workflows |
| 2026-09-08T08:43:55Z | PLAN-00003-STEP-15 | `uv run python -c "import yaml; yaml.safe_load(...)"` over ci.yml + publish.yml | YAML valid | unique-artifact-name fix applied to both workflows |
| 2026-09-08T08:43:55Z | PLAN-00003-STEP-15 | Read `actions/download-artifact` `dist/index.js` at pinned SHA `3e5f45b2…` | Confirmed: `name` input → `isSingleArtifactDownload` → `getArtifact(name)` returns exactly one (newest) artifact; `merge-multiple` only honoured in the `pattern`/`artifact-ids`/no-name branches | Root-cause evidence for the unique-artifact-name fix |
...
- **Completed requirements:** All 23 (REQ-01 through REQ-23)
- **Incomplete requirements:** None
- **Outstanding blockers:** None
- **Review request:** Ready — Builder execution is complete; the plan awaits independent Review or Validate, and the `v2.0.0rc1` release actions (merge, push, tag, GitHub release) remain the user's
