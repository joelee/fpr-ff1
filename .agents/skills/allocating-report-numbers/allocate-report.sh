...
# report number in a directory and create the empty report file.
#
...
# starting at 00001. Never fill a gap or reuse a number.
next=1
for f in "$dir"/[0-9][0-9][0-9][0-9][0-9]-*.md; do
  [ -e "$f" ] || continue
  base="$(basename "$f")"
  num="${base:0:5}"
  case "$num" in
...
# Create the empty file. Never overwrite an existing report.
if [ -e "$path" ]; then
