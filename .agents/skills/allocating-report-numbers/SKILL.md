...
creates the empty report file. It is directory-agnostic: any agent may point it
at any report directory (for example `docs/ideas/`, `docs/plans/`, or
`docs/reviews/`, or a new directory introduced later).
...
3. Scans `<directory>/[0-9][0-9][0-9][0-9][0-9]-*.md`, takes the highest leading
   five-digit number plus one (starting at `00001`), never filling a gap.
4. Creates the empty file `<directory>/NNNNN-<filename>`.
5. Releases the lock.
